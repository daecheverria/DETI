...
public class CambiarScena : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            // Cambia al n�mero de escena que desees, por ejemplo 1
            CambiarScenaBoton(2);
        }
    }
    ...
    IEnumerator FadePanel(int numeroEscena)
    {
        ...
        // Espera un peque�o tiempo para mostrar el panel (opcional)
        yield return new WaitForSeconds(0.5f);

        AsyncOperation asyncLoad = SceneManager.LoadSceneAsync(numeroEscena);
        yield return new WaitUntil(() => asyncLoad.isDone);

        // Aqu� puedes ocultar el panel si la escena no destruye este objeto
